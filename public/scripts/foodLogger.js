...
function renderDailyLog() {
    ...
    // --- Sync caloriesIn to server ---
    const username = localStorage.getItem('username');
    if (username && overallTotals.kcal > 0) {
        fetch('/logCalories', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                username,
                caloriesIn: Math.round(overallTotals.kcal),
                caloriesOut: 0  // Optionally update later from exercise page
            })
        })
        .then(res => {
            if (!res.ok) console.error("Failed to log calories:", res.statusText);
        })
        .catch(err => console.error("Error logging calories:", err));
    }
